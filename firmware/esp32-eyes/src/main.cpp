...
void renderHumanMouth(MouthShape shape, uint32_t now) {
  ...
  frame.fillRoundRect(24, 62, 192, 120, 40, rgb(1, 1, 4));
  frame.fillRoundRect(31, 69, 178, 106, 34, rgb(12, 10, 17));
  frame.drawRoundRect(32, 70, 176, 104, 33, rgb(35, 32, 44));
  frame.fillRoundRect(42, 82, 156, 82, 28, BLACK);

  if (shape == MouthShape::Sleep && !mouthState.talking) {
    frame.fillRoundRect(78, 123, 84, 7, 4, rgb(20, 76, 140));
    frame.drawFastHLine(88, 125, 64, rgb(90, 184, 244));
    return;
  }

  const int16_t w = int16_t(62.0f + pose.width * 104.0f);
  const int16_t h = int16_t(8.0f + pose.open * 76.0f);
  const int16_t cx = 120 + int16_t(pose.skew * 20.0f);
  ...
  frame.fillRoundRect(x - 15, y - 14, w + 30, h + 28, r + 14, rgb(0, 6, 28));
...
#if REACHY_SHARE_EYE_RST || (REACHY_HAS_MOUTH && (REACHY_MOUTH_RST == REACHY_LEFT_RST))
